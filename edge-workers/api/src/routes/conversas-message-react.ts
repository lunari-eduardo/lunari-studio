...
export async function conversasMessageReactRoute(c: Context<{ Bindings: Bindings }>) {
  ...
  const { reaction } = body;
  if (reaction === undefined) return c.json({ error: 'Reação não fornecida' }, 400);
  ...
  if (!msg.evolution_msg_id) return c.json({ error: 'Mensagem ainda não sincronizada com Evolution' }, 400);
  ...
  if (!chat?.contato_phone_normalized) return c.json({ error: 'Telefone do contato não disponível' }, 400);
  ...
  if (!instance?.instance_name) return c.json({ error: 'Instância não encontrada' }, 404);
