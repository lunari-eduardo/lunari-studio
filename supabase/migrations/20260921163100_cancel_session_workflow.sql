...
BEGIN
  ...
  IF p_action IN ('cancel_credit', 'cancel_refund', 'cancel_preserve') THEN
    IF v_session_status = 'cancelada' THEN
      RAISE EXCEPTION 'Sessão já se encontra cancelada.';
    END IF;
  END IF;
      ...
      IF p_action = 'cancel_credit' THEN
        ...
        IF v_total_paid > 0 THEN
          INSERT INTO cliente_creditos_ledger (
            ...
            v_user_id, v_cliente_id, v_total_paid, (now() AT TIME ZONE 'UTC')::date, 'cancelamento_sessao', 'Crédito de cancelamento de sessão', p_session_pk
          );
        END IF;
      ELSIF p_action = 'cancel_refund' THEN
        WITH inserted_estornos AS (
          ...
          SELECT
            ...
            'Estorno: ' || COALESCE(t.descricao, 'Pagamento') || ' (sessão cancelada)',
            v_user_id
